...
<h1 className={ns("noline cozy")} style={{marginTop: "8px"}}>Vale's Memory Model: A Hybrid of Swift, Rust, and Arcane Sorcery</h1 >
...
<div className={ns("section")}>
  <div className={ns("content cozy")}>Note that this is <b>still in development</b> and <i>very</i> untested. We're sharing the idea for early feedback, and to show you where Vale is headed. We'd love your thoughts, and let us know if you see any problems or have any questions!</div>
</div>

<div className={ns("section")}>
  <div className={ns("content cozy")}>And of course, if you want to be part of this and bring this into the world, come <a href="/contribute">come join us</a> and help make it happen!</div>
</div>
...
<a name="swift,rust,andarcanesorcery"></a><h2 >Swift, Rust, and Arcane Sorcery</h2 >
...
<ul className={ns("content cozy")}>
  <li className={ns()}>Swift's weak references decouple an object's destruction from its deallocation, a technique that can be reused to assist memory safety.</li>
  <li className={ns()}>In Rust, we can refer to an object with an index to an array that is paired with a "generation" number, which describes whether the object is still alive or not. We reuse this to implement zero-cost aliasing.</li>
  <li className={ns()}>We integrate these two things <i>into malloc() itself,</i> add a dash of static analysis, automatic borrow-checking, and low-level magic to create an entirely new approach for memory safety.</li>
</ul>

</div>

<div className={ns("section")}>
  <div className={ns("content cozy")}>First, let's get a new perspective where Rust's speed and safety come from, and see how we can take it apart and reassemble it.</div>
</div>

<a name="whatmakesrustfastandsafe?"></a><h2 >What makes Rust Fast and Safe?</h2 >
...
<ul className={ns("content cozy")}>
  <li className={ns()}>
    The borrow checker forces people to learn and use data-oriented programming instead of object-oriented programming. {this.noteAnchor("ecs")}
  </li>
  <li className={ns()}>
    Rust programs avoid expensive heap allocations.
  </li>
  <li className={ns()}>
    Isolating threads' memory from each other, allowing the non-atomic {incode("Rc<T>")} for reference counting. {this.noteAnchor("shared_ptr")}
  </li>
  <li className={ns()}>
    Generational indices into a central Vec. {this.noteAnchor("vector")}
  </li>
  <li className={ns()}>
...
<a name="wait,what'sagenerationalindex?"></a><h3  className={ns("noline")}>Wait, what's a Generational Index?</h3 >
...
<a name="objectandgenerationtables"></a><h4  className={ns("noline")}>Object and Generation Tables</h4 >
...
<a name="genind:ageneration+index"></a><h4  className={ns("noline")}>GenInd: A Generation + Index</h4 >
...
<a name="dereferencingagenind"></a><h4  className={ns("noline")}>Dereferencing a GenInd</h4 >
...
<a name="referencecounting"></a><h3  className={ns("noline")}>Reference Counting</h3 >

<div className={ns("section")}>
<div className={ns("content cozy")}>
Another source of Rust programs' speed is its {incode("Rc<T>")}. Whereas C++'s {incode("std::shared_ptr<T>")} is slow, {incode("Rc<T>")} is extremely fast, because it doesn't have to <b>atomically</b> increment and decrement the reference count integer.
</div>

<div className={ns("content cozy")}>
For example, when {incode("Rc<T>")} increments an integer (for example a {incode("u32")} named {incode("x")} containing the value 7, it does three simple things:
</div>
<ul className={ns("content cozy")}>
  <li className={ns()}>Load {incode("x")}'s value from RAM into a register.</li>
  <li className={ns()}>Add 1 to the register (now it's 8).</li>
  <li className={ns()}>Store the register's value back into {incode("x")} in RAM.</li>
</ul>

</div>

<a name="whydoesn'tc++'sshared_ptr&lt;t&gt;dothat?"></a><h4  className={ns("noline")}>Why doesn't C++'s shared_ptr&lt;T&gt; do that?</h4 >

<div className={ns("section")}>
<div className={ns("content cozy")}>
This works well for Rust, because all of the {incode("Rc<T>")}'s for a given object are guaranteed to be in the same thread. However, C++ lets us share {incode("std::shared_ptr<T>")}'s among multiple threads.
</div>
<div className={ns("content cozy")}>
Since multiple threads can run at the same time, we might get into an awkward situation like this:
</div>
<ul className={ns("content cozy")}>
  <li className={ns()}>(Thread A) Load {incode("x")}'s value from RAM into a thread A register.</li>
  <li className={ns()}>(Thread B) Load {incode("x")}'s value from RAM into a thread B register.</li>
  <li className={ns()}>(Thread B) Add 1 to the thread B register (now it's 8).</li>
  <li className={ns()}>(Thread A) Add 1 to the thread A register (now it's 8).</li>
  <li className={ns()}>(Thread A) Store thread A register's value back into {incode("x")} in RAM.</li>
  <li className={ns()}>(Thread B) Store thread B register's value back into {incode("x")} in RAM.</li>
</ul>
<div className={ns("content cozy")}>
...
<div className={ns("section")}>
<div className={ns("content cozy")}>
  Because Rust makes it so an object is only visible to one thread at a time, its {incode("Rc<T>")}'s increments and decrements dont need to be atomic, and are way faster.
</div>
</div>
...
<a name="canwegetthisspeedandsafetywithouttheborrowchecker?"></a><h3  className={ns("noline")}>Can we get this speed and safety without the borrow checker?</h3 >

<div className={ns("section")}>
<div className={ns("content cozy")}>
  Rust is an amazing language, though it's not perfect; a common lament is that <b>the borrow checker is difficult to work with.</b> {this.noteAnchor("fight")}
</div>
</div>
...
<div className={ns("section")}>
<div className={ns("content cozy")}>
On top of that, the language can employ non-atomic ref-counting and <b>automatic borrow checking</b> {this.noteAnchor("lbt")} to skip most liveness checks, to be even faster.
</div>
</div>
...
<Snippet header="Vale"><span class="Prog"><span class="Struct">struct <span class="StructName">GenRef</span><span class="IdentRunes">&lt;<span class="IdentRune">T</span>&gt;</span> <span class="Membs">&#123;<br />  <span class="Memb"><span class="MembName">targetGen</span> <span class="Typ">u48</span>;</span><br />  <span class="Memb"><span class="MembName">lgtIndex</span> <span class="Typ">u47</span>;</span><br />  <span class="Memb"><span class="MembName">objectPtr</span> <span class="Ownership">*<span class="Typ">T</span></span>;</span><br />&#125;</span></span><br /><br /><span class="Fn">fn <span class="FnName">makeGenRef</span><span class="IdentRunes">&lt;<span class="IdentRune">T</span>&gt;</span><span class="Params">(<span class="Pat"><span class="Capture"><span class="CaptureName">objectPtr</span></span> <span class="Ownership">*<span class="Typ">T</span></span></span>)</span><br /><span class="TplArgs"><span class="Typ">GenRef</span>&lt;<span class="Typ">T</span>&gt;</span> <span class="Block">&#123;<br />  <span class="Comment">// Get object's LGT index,</span><br />  <span class="Comment">// from 8b before objectPtr.</span><br />  <span class="Let"><span class="Pat"><span class="Capture"><span class="CaptureName">lgtIndex</span></span></span> = <span class="Lookup">...</span></span>;<br />  <span class="Comment">// Get object's generation</span><br />  <span class="Let"><span class="Pat"><span class="Capture"><span class="CaptureName">gen</span></span></span> = <span class="Call"><span class="Lookup">lgt</span>[<span class="MemberAccess"><span class="Lookup">lgtIndex</span><span class="MemberAccess">.</span><span class="Lookup">0</span></span>]</span></span>;<br />  <span class="Comment">// Make the GenRef!</span><br />  <span class="Ret">ret <span class="Call"><span class="CallLookup">GenRef</span>(<br />    <span class="Lookup">gen</span>, <span class="Lookup">lgtIndex</span>, <span class="Lookup">objectPtr</span>)</span>;</span><br />&#125;</span></span><br /></span>
</Snippet>
</div>
</div>
...
<b>Vale's Memory Model: A Hybrid of Swift, Rust, and Arcane Sorcery</b><ul className={ns("c-toc")}><li className={ns("c-toc")}><a href="/blog/next-gen-memory-model#swift,rust,andarcanesorcery">Swift, Rust, and Arcane Sorcery</a></li><li className={ns("c-toc")}><a href="/blog/next-gen-memory-model#whatmakesrustfastandsafe?">What makes Rust Fast and Safe?</a></li><ul className={ns("c-toc")}><li className={ns("c-toc")}><a href="/blog/next-gen-memory-model#wait,what'sagenerationalindex?">Wait, what's a Generational Index?</a></li></ul><ul className={ns("c-toc")}><li className={ns("c-toc")}><a href="/blog/next-gen-memory-model#referencecounting">Reference Counting</a></li></ul><ul className={ns("c-toc")}><li className={ns("c-toc")}><a href="/blog/next-gen-memory-model#canwegetthisspeedandsafetywithouttheborrowchecker?">Can we get this speed and safety without the borrow checker?</a></li></ul><li className={ns("c-toc")}><a href="/blog/next-gen-memory-model#stage1:asimplebeginning">Stage 1: A Simple Beginning</a></li><li className={ns("c-toc")}><a href="/blog/next-gen-memory-model#stage2:addinggenerationnumbers">Stage 2: Adding Generation Numbers</a></li><li className={ns("c-toc")}><a href="/blog/next-gen-memory-model#nextsteps">Next Steps</a></li><li className={ns("c-toc")}><a href="/blog/next-gen-memory-model#afterword:anarcanetellingofwhat'stocome">Afterword: An Arcane Telling of What's to Come</a></li></ul>
                </div>
...
<Note name="shared_ptr" iconsAndPositions={this.state.noteIconsAndPositions} update={this.updateNoteSizeAndCustomIcon}>
  Rust's {incode("Rc<RefCell<T>>")} is like C++'s {incode("std::shared_ptr<T>")}.
</Note>
...
<Note name="vector" iconsAndPositions={this.state.noteIconsAndPositions} update={this.updateNoteSizeAndCustomIcon}>
  Rust's {incode("Vec<T>")} is like C++'s {incode("std::vector<T>")}.
</Note>
...
<Note name="ecs" iconsAndPositions={this.state.noteIconsAndPositions} update={this.updateNoteSizeAndCustomIcon}>
...
<Note name="fight" iconsAndPositions={this.state.noteIconsAndPositions} update={this.updateNoteSizeAndCustomIcon}>
...
<Note name="lbt" iconsAndPositions={this.state.noteIconsAndPositions} update={this.updateNoteSizeAndCustomIcon}>
