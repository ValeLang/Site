...
<div className={ns("section")}>
  <div className={ns("content cozy")}>Vale aims to be as fast as C++, as safe as Java, and easy to learn. Using a novel combination of single ownership, reference counting, region borrow checking, and an arsenal of other optimizations, Vale can reduce ref-counting overhead lower than ever before.</div>
  <div className={ns("content cozy")}>Any safe memory management strategy has run-time overhead. Garbage collected languages (Java, Javascript) must "stop the world" for milliseconds at a time, {this.noteAnchor("630")} and reference counting languages (Python, Swift, ObjC, some C++) pay costs in incrementing and decrementing ref counts. Even borrow checking languages (Rust, Cyclone) pay RC or Vec costs, when the borrow checker forces us into them; memory safety has a run-time cost, no matter what language you're in. {this.noteAnchor("915")}</div>
  <div className={ns("content cozy")}>Vale is no exception; its Normal Mode and Resilient Mode guarantee memory safety, but have some minor overhead at run-time. {this.noteAnchor("1138")} However, Vale's <b>region borrow checking, compile time ref-counting, constraint references, memory pools, and bump allocation</b> work together to drastically reduce the overhead, giving us speed with perfect memory safety.</div>
  <div className={ns("content cozy")}>v0.2 will be the regions release, where add support for all of these features. We're still designing it and seeing if it will work! Let us know if you have any questions or ideas, or if you see any opportunities or mistakes!</div>
</div>     

<a name="referencecounting"></a><h2 className={ns()}>Reference Counting</h2>

<div className={ns("section")}>
  <div className={ns("content cozy")}>Vale uses reference counting to guarantee safety: whenever we make a new reference to an object, we must increment that object's <b>reference count</b>, {this.noteAnchor("930")} and when that reference goes away, we must decrement it again. We can't deallocate an object until its reference count is zero.</div>
  <div className={ns("content cozy")}>The first optimization that will help with this is called <b>compile-time reference counting</b>. It was invented by Wouter van Oortmerssen for Lobster, and it uses Rust-inspired lifetime analysis to eliminate 95% of increments and decrements at compile time, leaving only 5% to happen at run-time. {this.noteAnchor("627")}</div>
...
<a name="cycles"></a><h3 className={ns()}>Cycles</h3>

<div className={ns("content cozy")}>The first weakness of RC is that it can form cycles, causing memory leaks. Vale doesn't have this problem because every object has one owning reference, and it enforces that no other references are alive when we let go of the owning reference.</div>

<a name="atomicity"></a><h3 className={ns()}>Atomicity</h3>
  ...
  <div className={ns("content cozy")}>In Vale, an object can only be modified by one thread at a time, so a program can have threads and still use non-atomic ref-counting.</div>

  <a name="branchmispredictions"></a><h3 className={ns()}>Branch Mispredictions</h3>
  ...
  <a name="cachemisses"></a><h3 className={ns()}>Cache Misses</h3>
  ...
  <a name="readonlyregions"></a><h2 className={ns()}>Read-Only Regions</h2>
  ...
  <a name="implicitlocking"></a><h3 className={ns()}>Implicit Locking</h3>

  <div className={ns("content cozy")}>Programs often have "pure" functions, where a function reads the outside world through its parameters, does a bit of calculation (perhaps modifying some of its own locals along the way), and then returns a result, all without modifying the outside world. In Vale, we can annotate a function with the <b>pure</b> keyword to make the compiler enforce this. This is a common design pattern, and leads to much more maintainable and testable code.</div>

  <div className={ns("content cozy")}>If we add <b>region annotations</b> to our pure function, Vale will <b>implicitly lock</b> all existing memory, thus making references to any existing memory <b>completely free;</b> we don't have to increment or decrement anything because all these references are temporary anyway. Below, we use region annotations (highlighted) to tell the compiler which references point to the outside world.</div>
  ...
  <div className={ns("content splitter")}>
    <div className={ns("half")}>
      ...
      <div className={ns("content cozy")}>A {incode("Desire")} describes what the unit could do, and how much it wants to do that.</div>
      <div className={ns("content cozy")}>When we have all the {incode("Desire", "s")}, we sort them to figure out what the strongest one is, and enact it.</div>
    </div>
    <div className={ns("half")}>
...
<div className={ns("content splitter")}>
  <div className={ns("half")}>
    ...
    <div className={ns("content cozy")}>This function doesn't change anything about itself or the unit or the world, it just reads them and does calculations.</div>
...
<a name="explicitlocking"></a><h3 className={ns()}>Explicit Locking</h3>

<div className={ns("content cozy")}>Implicit locking locked all existing memory, and made a small new region called 'i which we could modify. There's a more precise way to manage regions: mutexes! {this.noteAnchor("152")}</div>
...
<div className={ns("content cozy")}>One of those is the "Templar" stage, which reads the "astrouts" AST and builds the "temputs" AST. We can put the astrouts in a Mutex, and the temputs in another Mutex. The Templar gets read-only access to the astrouts mutex, while it uses it's read-write access to the temputs mutex to build it up.</div>

<div className={ns("content splitter")}>
  <div className={ns("half")}>
    <div className={ns("content cozy")}>Here, the {incode("templar")} function takes in the {incode("astroutsMutex")}.</div>
    <div className={ns("content cozy")}>The {incode("astroutsMutex")} starts closed, so we call {incode("openro")} to open it for read-only access.</div>
    <div className={ns("content cozy")}>We then create a new Mutex containing an empty Temputs. We immediately open it in read-write mode.</div>
    <div className={ns("content cozy")}>We give both the temputs and a function from the astrouts to translateFunction, so it can make a translated function and add it to temputs.</div>
    <div className={ns("content cozy")}>At the end of {incode("templar")}, the locks are dropped, automatically closing the mutexes, and we return the now-closed {incode("temputsMutex")}.</div>

    <div className={ns("content cozy")}>With our {incode("Mutexes")} and region annotations, the compiler can give us free, zero-cost access to everything in the astrouts.</div>

  </div>
  <div className={ns("half")}>

<Snippet header="Vale"><span class="Prog"><span class="Fn">fn <span class="FnName">templar</span><span class="Params">(<br />    <span class="Pat"><span class="Capture"><span class="CaptureName">astroutsMutex</span></span> <span class="Ownership">&<span class="Permission">!<span class="TplArgs"><span class="Typ">Mutex</span>&lt;<span class="Typ">Astrouts</span>&gt;</span></span></span></span>)</span> <span class="Block">&#123;<br />  <span class="Let"><span class="Pat"><span class="Capture"><span class="CaptureName">astroutsLock</span></span></span> = <span class="Call"><span class="Lookup">astroutsMutex</span><span class="MemberAccess">.</span><span class="CallLookup">openro</span>()</span></span>;<br />  <span class="Let"><span class="Pat"><span class="Capture"><span class="CaptureName">astrouts</span></span></span> = <span class="MemberAccess"><span class="Lookup">astroutsLock</span><span class="MemberAccess">.</span><span class="Lookup">contents</span></span></span>;<br /><br />  <span class="Let"><span class="Pat"><span class="Capture"><span class="CaptureName">temputsMutex</span></span></span> = <span class="Call"><span class="CallLookup">Mutex</span>(<span class="Lambda"><span class="Block">&#123; <span class="Call"><span class="CallLookup">Temputs</span>()</span> &#125;</span></span>)</span></span>; <br />  <span class="Let"><span class="Pat"><span class="Capture"><span class="CaptureName">temputsLock</span></span></span> =<br />    <span class="Call"><span class="Lookup">temputsMutex</span><span class="MemberAccess">.</span><span class="CallLookup">openrw</span>()</span></span>; <span class="Comment">{this.noteAnchor("345")}</span><br />  <span class="Let"><span class="Pat"><span class="Capture"><span class="CaptureName">temputs</span></span></span> = <span class="MemberAccess"><span class="Lookup">temputsLock</span><span class="MemberAccess">.</span><span class="Lookup">contents</span></span></span>;<br /><br />  <span class="Call"><span class="CallLookup">translateFunction</span>(<br />      <span class="Call"><span class="MemberAccess"><span class="Lookup">astrouts</span><span class="MemberAccess">.</span><span class="Lookup">functions</span></span>[<span class="Num">0</span>]</span>, <span class="Lend">&<span class="Lookup">temputs</span></span>)</span>;<br /><br />  <span class="Lookup">...</span>;<br /><br />  <span class="Ret">ret <span class="Lookup">temputsMutex</span>;</span><br />&#125;</span></span><br /><br /><span class="Fn">fn <span class="FnName">translateFunction</span><span class="IdentRunes">&lt;<span class="IdentRune">'a ro</span>, <span class="IdentRune">'t</span>&gt;</span><span class="Params">(<br />    <span class="Pat"><span class="Capture"><span class="CaptureName">func</span></span> 'a <span class="Ownership">&<span class="Typ">AFunction</span></span></span>,<br />    <span class="Pat"><span class="Capture"><span class="CaptureName">temputs</span></span> 't <span class="Ownership">&<span class="Permission">!<span class="Typ">Temputs</span></span></span></span>)</span> <span class="Typ">TFunction</span> <span class="Block">&#123;<br />  <span class="Comment">// Read func, add things to temputs.</span><br />  <span class="Lookup">...</span>;<br />&#125;</span></span><br /></span>
</Snippet>
  </div>
</div>

<div className={ns("content cozy")}>We still increment and decrement the ref-counts of objects inside 'i, but we just made those objects, so they'll likely be hot in the cache.</div>
...
<a name="regionmemorystrategy"></a><h2 className={ns()}>Region Memory Strategy</h2>

<div className={ns("content cozy")}>In our example above, references into {incode("'r")} were completely free. And references into {incode("'i")} were probably hot in the cache, making its reference counting very fast.</div>
<div className={ns("content cozy")}>How much more can we do? <b>Much more.</b> This is where things get a bit crazy.</div>

<div className={ns("content cozy")}>Vale's <b>pool and arena allocation</b> can eliminate the ref-counting overhead in {incode("'i")} too, and <b>lets eliminate its malloc and free overhead as well, while we're at it.</b></div>
<div className={ns("content cozy")}>The default memory management strategy for a region is to use the <b>heap</b>, which uses malloc and free under the hood.</div>
<div className={ns("content cozy")}>We can make it so a certain region uses <b>pool</b> allocation, which is <i>much</i> faster. Pool allocation will cache all freed structs for future allocations of the same type. {this.noteAnchor("452")}</div>

<div className={ns("content cozy")}>Functions can also use <b>arena</b> allocation, where we instead use a large "slab" of memory, and we keep allocating from the next part of it. Whenever it runs out, Vale allocates another slab of memory. {this.noteAnchor("754")} This can push performance even faster, though one should be careful when using this, as it could destroy the heap. Fun fact: the pool allocator is built on top of the arena allocator, under the hood.</div>
...
<div className={ns("content cozy")}>The only memory overhead we pay is when we copy {incode("findNearbyUnits", "'s")} {incode("'i List<'r &Unit>")} result from the pool region into the caller's region.</div>
<div className={ns("content cozy")}>Because Vale makes it so easy to optimize with pool allocation, Vale could become the obvious choice for performance-critical software.</div>


<a name="fastandsafe"></a><h2 className={ns()}>Fast and Safe</h2>


<div className={ns("content cozy")}>Vale uses single ownership and region isolation to optimize its reference counting, and then offers read-only regions and pool-allocation for when we want to eliminate it altogether.</div>
<div className={ns("content cozy")}>With these, Vale can be 100% safe, while also being one of the fastest languages in existence.</div>
...
<b>Zero-Cost References with Regions in Vale</b><ul><li><a href="/blog/zero-cost-refs-regions#referencecounting">Reference Counting</a></li><ul><li><a href="/blog/zero-cost-refs-regions#cycles">Cycles</a></li></ul><ul><li><a href="/blog/zero-cost-refs-regions#atomicity">Atomicity</a></li></ul><ul><li><a href="/blog/zero-cost-refs-regions#branchmispredictions">Branch Mispredictions</a></li></ul><ul><li><a href="/blog/zero-cost-refs-regions#cachemisses">Cache Misses</a></li></ul><li><a href="/blog/zero-cost-refs-regions#readonlyregions">Read-Only Regions</a></li><ul><li><a href="/blog/zero-cost-refs-regions#implicitlocking">Implicit Locking</a></li></ul><ul><li><a href="/blog/zero-cost-refs-regions#explicitlocking">Explicit Locking</a></li></ul><li><a href="/blog/zero-cost-refs-regions#regionmemorystrategy">Region Memory Strategy</a></li><li><a href="/blog/zero-cost-refs-regions#fastandsafe">Fast and Safe</a></li></ul>
                </div>
...
<Note name="1138" iconsAndPositions={this.state.noteIconsAndPositions} update={this.updateNoteSizeAndCustomIcon}>
  <div className={ns("content cozy")}>Some applications choose to use Fast Mode which has zero overhead, and gain more confidence in its safety by developing and testing more in Normal Mode.</div>
  <div className={ns("content cozy")}>If we're looking for 100% certainty though, we must use Normal Mode or Resilient Mode.</div>
</Note>
...
<Note name="1136" iconsAndPositions={this.state.noteIconsAndPositions} update={this.updateNoteSizeAndCustomIcon}>
  <div className={ns("content cozy")}>This is the "map" operator, it calls the method on the right on all the elements of the collection on the left.</div>
